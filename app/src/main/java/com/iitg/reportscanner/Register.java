    ...
    private void register_user(String email, String password, final String name, final String age) {
        ...
        mAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener(task -> {
            ...
            if (task.isSuccessful()) {
                ...
                FirebaseInstanceId.getInstance().getInstanceId()
                        .addOnCompleteListener(task12 -> {
                            if (!task12.isSuccessful()) {
                                Log.w("TAG", "getInstanceId failed", task12.getException());
                                return;
                            }

                            // Get new Instance ID token
                            device_token[0] = Objects.requireNonNull(task12.getResult()).getToken();
                ...
                userMap.put("device_token", device_token[0]);
